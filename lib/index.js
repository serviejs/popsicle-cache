  ...
  return function (request, next) {
    ...
    return next().then(function (response) {
      ...
      if (_cache && response.status === 304) {
        ...
        response.statusText = _cache.response.statusText
        response.body = _cache.response.body
        response.headers = _cache.response.headers
        response.rawHeaders = _cache.response.rawHeaders
      ...
      if (!isCacheableResponse(request, response, now)) {
        if (memoryCache) {
          memoryCache.del(key)
        }

        return store.del(key).then(() => response)
      }
      ...
      return store.set(key, data).then(() => response)
    })
  }
}
 ...
 * @return {boolean}
 */
function isCacheableResponse (request, response, now) {
